...
func (a *ibmmqsourceAdapter) Start(ctx context.Context) error {
	...
	conn, err := mq.NewConnection(a.mqEnvs)
	if err != nil {
		return fmt.Errorf("failed to create IBM MQ connection: %w", err)
	}
	defer conn.Disc()

	queue, err := mq.OpenQueueToRead(a.mqEnvs.QueueName, conn)
	if err != nil {
		return fmt.Errorf("failed to open IBM MQ queue: %w", err)
	}
	defer queue.Close()

	if err := queue.RegisterCallback(a.eventHandler()); err != nil {
...
func (a *ibmmqsourceAdapter) eventHandler() mq.Handler {
	return func(data []byte, correlID string) error {
		...
		event.SetSource(fmt.Sprintf("%s/%s", a.mqEnvs.ConnectionName, strings.ToLower(a.mqEnvs.ChannelName)))
		if correlID != "" {
